...
export function useTabNavFunction(
      ...
      setTimeout(() => {
        ...
        if (tabEle && tabsEle.value) {
          if (['top', 'bottom'].includes(props.tabPosition)) {
            ...
            data.offsetTop =
              tabEle.getBoundingClientRect().top -
              tabsEle.value.getBoundingClientRect().top;
  ...
  const canChange = (currentTab: Active) => {
    ...
    if (typeof props.beforeChange === 'function') {
      const result = props.beforeChange(currentTab) as
        | boolean
        | Promise<boolean>;
  ...
  const activeClick = (item: TabsStateData, tabEl?: Element) => {
    ...
    canChange(id).then((change) => {
      ...
      const tab = tabsList.value.find(
        (itemOption) => itemOption.props.id === id
      );
        ...
        if (props.type === 'slider' && tabEl && tabsEle && tabsEle.value) {
          if (['left', 'right'].includes(props.tabPosition)) {
            data.offsetLeft =
              ...
              tabsEle.value.nativeElement.getBoundingClientRect().left;
          } else {
            data.offsetTop =
              tabEl.getBoundingClientRect().top -
              tabsEle.value.nativeElement.getBoundingClientRect().top;
