...
import { EventType, RippleOptions, RippleProto } from './ripple-types';
import './ripple.less';
...
const RippleDirective: Directive & RippleProto = {
  beforeMount: (el: HTMLElement, binding) => {
    ...
    const color = binding?.value?.color || '#00000050';
    const duration = binding?.value?.duration || 200;
    const index = binding?.value?.zIndex;
    const opacity = binding?.value ? 0.2 : 0.3;
    options.transition = duration;
    const bg = el.getAttribute('ripple-background') || color;
    setProps(Object.keys(binding.modifiers), options);

    const background = bg || RippleDirective.background;
    const zIndex = RippleDirective.zIndex || index;
