...
import dresize, { ResizeDirectiveProp } from '../g-resize-directive';
...
import { isHTMLElement, prefix } from '../../../shared/utils';
import './splitter-bar.scss';
import { useI18n } from '../../../locale/create';

export default defineComponent({
  name: `${prefix}SplitterBar`,
  components: {
    Tooltip,
  },
  ...
  setup(props: SplitterBarProps) {
    ...
    const renderCollapsedTip = () => {
      if (!props || props.index === undefined) {
        return t('splitterBar.collapse');
      }
      const { pane, nearPane } = queryPanes(props.index, props.index + 1);
      const isCollapsed = pane?.component?.props?.collapsed || nearPane?.component?.props?.collapsed;
      return isCollapsed ? t('splitterBar.expand') : t('splitterBar.collapse');
    };
      ...
      return withDirectives(
        <div class={state.wrapperClass} ref={domRef}>
          {props.showCollapseButton && (
            <Tooltip title={renderCollapsedTip()}>
              <div
